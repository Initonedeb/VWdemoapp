...
const EmployeeDashboard: React.FC<EmployeeDashboardProps> = ({ onLogout }) => {
  const [view, setView] = useState<'dashboard' | 'cars' | 'clients' | 'prestamos'>('dashboard');
  
  if (view === 'cars') {
    return <UsedCarManagement onBack={() => setView('dashboard')} />;
  }
  
  if (view === 'clients') {
    return <ClientManagement onBack={() => setView('dashboard')} />;
  }

  if (view === 'prestamos') {
    return <PrestacionesManagement onBack={() => setView('dashboard')} />;
  }
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      <header className="flex justify-between items-center mb-10">
        <div>
          <h1 className="text-3xl font-bold text-blue-900">Panel de Empleado</h1>
      ...
      <main className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div 
          ...
          className="group bg-gray-50 border border-gray-200 rounded-2xl p-8 shadow-lg hover:shadow-blue-500/20 transition-all duration-300 ease-in-out transform hover:-translate-y-2 cursor-pointer"
        >
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-gray-600">Gestionar el inventario de vehículos de demostración.</p>
          </div>
        </div>
